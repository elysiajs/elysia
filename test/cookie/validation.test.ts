...
describe('Cookie Validation', () => {
	it('validate required cookie', async () => {
		const app = new Elysia().get('/', ({ cookie: { session } }) => session.value, {
			cookie: t.Cookie({
				session: t.String()
			})
		})
	...
	it('validate cookie type - boolean', async () => {
		const app = new Elysia().get('/', ({ cookie: { active } }) => active.value, {
			cookie: t.Cookie({
	...
	it('validate cookie with object schema', async () => {
		...
		const valid = await app.handle(
			req('/', {
				headers: {
					Cookie: 'profile=' + encodeURIComponent(JSON.stringify({ name: 'Himari', age: 16 }))
				}
			})
		)
		...
		const invalid = await app.handle(
			req('/', {
				headers: {
					Cookie: 'profile=' + encodeURIComponent(JSON.stringify({ name: 'Himari' }))
				}
			})
		)
	...
	it('validate multiple cookies', async () => {
		...
		const [valid, missingSession, missingUserId, invalidUserId] = await Promise.all([
			app.handle(
				req('/', {
	...
	it('validate cookie with transform', async () => {
		const app = new Elysia().get(
			...
			{
				cookie: t.Cookie({
					timestamp: t.Transform(t.String())
	...
	it('validate cookie with array type', async () => {
		...
		const response = await app.handle(
			req('/', {
				headers: {
					Cookie: 'tags=' + encodeURIComponent(JSON.stringify(['tag1', 'tag2', 'tag3']))
				}
			})
		)
	...
	it('validate cookie with union type', async () => {
		...
		const [stringValue, numericValue, invalid] = await Promise.all([
			...
			app.handle(
				req('/', {
					headers: { Cookie: 'value=' + encodeURIComponent(JSON.stringify({ obj: true })) }
				})
			)
		])
	...
	it('inherits cookie validation on guard', async () => {
		const app = new Elysia()
			...
			.get('/profile', ({ cookie: { session } }) => `Profile: ${session.value}`)
	...
	it('validate empty cookie object when optional', async () => {
		const app = new Elysia().get(
			'/',
			({ cookie }) => (Object.keys(cookie).length === 0 ? 'empty' : 'not empty'),
			{
				cookie: t.Optional(
					t.Cookie({
						session: t.Optional(t.String())
