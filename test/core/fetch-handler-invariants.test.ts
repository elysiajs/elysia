...
// Fetch-handler and error-tail regressions.
// Each test fails on the pre-fix code and passes after the fix.

describe('fetch-handler invariants', () => {
	// findRoute must not crash when the router is undefined.
	...
	// a returning error handler whose value is a plain domain object that
	...
	//  (companion) — an error handler that DOES return status must still
	...
	//  (sync) — a short-circuiting request hook must still invoke
	...
	//  (async) — same, when an async request hook is present (forces the
	...
	// the default 404 must still emit `Elysia.headers` defaults instead of
	...
	// headers written by a request hook must also survive the 404.
	...
	// when an error hook handles NotFound and sets a different status,
