...
describe('Macro', () => {
	...
	it('introspect: macro defined inside groups', () => {
		const data: {
			introspectOptions: any
			metadata: any
		}[] = []
	...
	it('introspect: macro called inside group', () => {
		const data: {
			introspectOptions: any
			metadata: any
		}[] = []
		...
		app.macro({
			...
			group
				.guard({ myMacro: true })
				.get('/users/:id', () => 'Hello from users/:id')
		)

		expect(data).toEqual([
			{
				introspectOptions: { myMacro: true },
				metadata: {
					path: '/api/users/:id',
					method: 'GET'
				}
	...
	it('introspect: not run for macro keys set to false', () => {
		...
		// No call with { a: false }
	})

	it('guard({ macro: false }) clears stale introspection state', () => {
		// Test that disabling a macro in a guard removes it from guardMacroOptions
		// so subsequent routes don't introspect with stale { macro: true }
		const calls: {
			introspectOptions: any
			metadata: any
		}[] = []
		...
		const plugin = new Elysia({ name: 'macro-plugin' }).macro({
			a: {
				introspect(introspectOptions, metadata) {
					calls.push({ introspectOptions, metadata })
				}
			}
		})
	...
	it('guard({ macro: false }) prevents stale introspection on route after guard disabled', () => {
		const calls: {
			introspectOptions: any
			metadata: any
		}[] = []
		...
		const app = new Elysia()
			.use(plugin)
			.guard({ a: true }, (app) => app.get('/route1', () => 'route1'))
			.guard({ a: false }, (app) => app.get('/route2', () => 'route2'))
			.get('/route3', () => 'route3')

		// Route1 should introspect with { a: true }
		// Route2 and route3 should NOT introspect with stale { a: true }
		// Only route1 should have introspected
		expect(calls.length).toBe(1)
		expect(calls[0]?.introspectOptions).toEqual({ a: true })
		expect(calls[0]?.metadata.path).toBe('/route1')
		// Route2 and route3 should not have introspected at all since the macro was disabled
		// If the bug exists, they would introspect with stale { a: true }
	})
})
