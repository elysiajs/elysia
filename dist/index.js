...
export default class Elysia {
    ...
    _addHandler(method, _path, handler, hook) {
        ...
        const header = getSchemaValidator(hook?.schema?.header ?? this.$schema?.header, true);
        ...
        if (handler.validator) {
            ...
            if (validator.header) {
                ...
                if (validator.header.Check(_header) === false)
                    throw createValidationError('header', validator.header, _header);
            }
            if (validator.params &&
    ...
    handleError(error) {
        for (let i = 0; i < this.event.error.length; i++) {
            const response = this.event.error[i](mapErrorCode(error.message), error);
