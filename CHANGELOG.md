...
Behavior Change:
- [Type] guard schemas are now typed by channel, matching the runtime: a bare `.guard({ … })` is OVERRIDE — the closer to the route, the more power, so a nearer guard or the route-local schema replaces its fields (response per status code); `.guard({ schema: 'standalone', … })` opts into ADDITIVE validation where every visible standalone schema must be satisfied (intersect). Previously the types treated every guard schema as standalone-intersect
- [Type] instance-level `.transform()` no longer inherits ANY guard input schema — its context is typed as raw, pre-validation values (path params from the instance prefix are kept)
