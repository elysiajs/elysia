...
Behavior Change:
- EVERY `.guard()` form now defaults to the OVERRIDE channel — the closer to the route, the more power: a nearer guard's schema or the route-local schema replaces an inherited one per field (response per status code). Additive validation (every visible validator runs as its own pass) requires an explicit `schema: 'standalone'` opt-in. Previously the string-scope forms (`.guard('local' | 'plugin' | 'global', …)`) forced `standalone` implicitly while the object forms defaulted to override — the same method defaulted differently depending on call shape
