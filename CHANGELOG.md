...
- a handler-returned `Response` is now passed through by reference when nothing about `set` would change it (untouched status/headers/cookie), instead of always being rewrapped into a fresh `Response` — this preserves an in-memory body and its `content-length` (previously a rewrap converted it to a stream-backed body and dropped `content-length`). On Bun, a touched `set` merges its headers into the returned `Response` **in place** (the response's own header wins on a conflict; `set-cookie` always forces a rewrap). Caveat: if a handler returns the **same** `Response` object across requests (e.g. a module-level constant) while writing per-request `set.headers`, those headers now mutate the shared object and leak to later requests (previously this errored loudly with "ReadableStream already used"). Return a fresh `Response` per request, or set headers via `set.headers`, to avoid this
