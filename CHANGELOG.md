...
- an ordinary handler- or hook-returned `Response` is request-owned and single-use. An untouched response may pass through by reference; when response state must be applied, Elysia transfers its body into the patched response without clone-and-pump overhead. Return a fresh `Response`/factory result per request, use a static route value, or wrap an explicitly shared response with `borrow(response)`; silently returning the same unmarked response again now reaches the normal 500 error path after its body has been consumed. Owned streaming responses preserve their declared headers exactly, so a headerless `transfer-encoding: chunked` response no longer gains `application/octet-stream` as a side effect of JS re-pumping; set the type explicitly or return `bytes(stream)` for the certified binary lane
