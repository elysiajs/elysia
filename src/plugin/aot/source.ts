...
export async function captureArtifacts(
	...
	try {
		...
		const fingerprint = app['~aotFingerprint'] ?? createAotFingerprint()
...
function emitModule(
	...
	handlers: CapturedHandler[],
	fingerprint: AotFingerprint,
	options: CompileToSourceOptions | undefined
): string {
	...
	const treeToSource = (
		...
		const slotRef = new Map<string, string>()
		...
		for (const method in tree) {
			...
			for (const path in tree[method]) {
				...
				if (ref === undefined) {
					...
					validatorDecls += `const ${ref} = ${slotObj}\n`
				}
	...
	if (lazy) {
		...
		const treeStr = treeToSource(tree)
	...
	const moduleCondition = options?.moduleCondition ?? 'esm'
	...
	if (needs('CheckContext'))
		body += importNamed('CheckContext', 'typebox/schema')
	if (needs('Guard')) body += importNamed('Guard', 'typebox/guard')
