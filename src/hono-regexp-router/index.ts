...
// Using Hono Tire Router as it's the fastest in Bun
...
function compareRoute<T>(a: Route<T>, b: Route<T>): CompareResult {
	...
	return i === b.hint.regExpComponents.length || a.hint.endWithWildcard
		? 1
		: 0
}

function compareHandler(
	a: HandlerWithSortIndex<any>,
	b: HandlerWithSortIndex<any>
) {
...
function getSortedHandlers<T>(
	handlers:
		| HandlerWithSortIndex<T>[]
		| IterableIterator<HandlerWithSortIndex<T>>
): T[] {
...
function buildMatcherFromPreprocessedRoutes<T>(
	...
	for (let i = 0, len = routes.length; i < len; i++) {
		...
		if (!hasAmbiguous) {
			handlers[i][0] = getSortedHandlers(
				handlers[i][0] as HandlerWithSortIndex<T>[]
			)
		}
	}
...
function verifyDuplicateParam<T>(routes: Route<T>[]): boolean {
	...
	for (let i = 0, len = routes.length; i < len; i++) {
		...
		for (let k = 0, len = paramAliasMapKeys.length; k < len; k++) {
			...
			for (
				let l = 0, len = paramAliasMap[aliasFrom].length;
				l < len;
				l++
			) {
...
export default class RegExpRouter<T> implements Router<T> {
	routeData?: {
		index: number
		routes: Route<T>[]
		methods: Set<string>
	...
	match(method: string, pathname: string): PathResult<T> | null {
		const [path, stringifiedQuery] = parseUrl(pathname)
		const result = this._match(method, path)
		...
		return [
			result[0][0],
			result[1],
			stringifiedQuery ? parseQuery(stringifiedQuery) : emptyParam
		] as unknown as PathResult<T>
	}

	private _match(method: string, pathname: string): Result<T> | null {
		const [primaryMatchers, secondaryMatchers, hasAmbiguous] =
			this.buildAllMatchers()

		this._match = hasAmbiguous
			? (method, path): Result<T> | null => {
					const matcher = (primaryMatchers[method] ||
						primaryMatchers[
							METHOD_NAME_ALL
						]) as MatcherWithSortIndex<T>
					...
					return [getSortedHandlers(handlers.values()), params]
			  }
			: (method, path): Result<T> | null => {
					let matcher = (primaryMatchers[method] ||
						primaryMatchers[METHOD_NAME_ALL]) as Matcher<T>
					let match = path.match(matcher[0])

					if (!match) {
						const matchers =
							secondaryMatchers[method] ||
							secondaryMatchers[METHOD_NAME_ALL]
						for (
							let i = 0, len = matchers.length;
							i < len && !match;
							i++
						) {
		...
		return this._match(method, pathname)
	}
		...
		this.routeData.routes.sort(({ hint: a }, { hint: b }) => {
			...
			for (
				let i = 0,
					len =
						Math.min(
							a.paramIndexList.length,
							b.paramIndexList.length
						) + 1;
				i < len;
				i++
			) {
	...
	private buildMatcher(
				...
				if (compareResult === 1) {
					...
					for (let k = 0, len = namedParams.length; k < len; k++) {
						...
						if (m) {
							...
							routes[j].paramAliasMap[m[1]].push(
								namedParams[k][1]
							)
						} else {
							components[namedParams[k][0]] = `/:${
								namedParams[k][1]
							}{${c.substring(1)}}`
					...
					if (
						routes[j].hint.components.length <
						routes[i].hint.components.length
					) {
		...
		for (let i = 0, len = routes.length; i < len; i++) {
			if (
				routes[i].hint.maybeHandler ||
				!routes[i].hint.endWithWildcard
			) {
...
export type { HTTPMethod, Result } from './types'
