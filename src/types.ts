...
/**
 * Response schema of an error handler.
 *
 * Unlike a normal handler, the status of a plain return value is decided by
 * the error that triggered `onError` (`NOT_FOUND` 404, `PARSE` 400,
 * `VALIDATION` 422, `INTERNAL_SERVER_ERROR` 500), not by 200.
 *
 * Explicit `status(...)` / `error(...)` returns keep their exact code via
