...
export default class Elysia<Instance extends ElysiaInstance = ElysiaInstance> {
  ...
  private add(
    ...
    const validator = {
      body: getSchemaValidator(hook?.body ?? (this.$schema?.body as any), defs),
      ...
      this.lazyLoadModules.push(
        plugin
          .then((plugin) => {
            ...
            return plugin.default(this as unknown as any) as unknown as Elysia
          })
          .then((x) => x.compile())
      )
        ...
        if (
          ...
          this.server?.upgrade(context.request, {
            ...
            data: {
              ...
              message: getSchemaValidator(options?.body, this.meta[DEFS]),
  ...
  state(
    ...
    if (!(name in this.store)) {
      ;(this.store as Record<string | number | symbol, unknown>)[name] = value
    }
  ...
  derive<Returned extends Object = Object>(
    transform: (
      context: Context<
        TypedSchemaToRoute<Instance['schema'], Instance['meta'][typeof DEFS]>,
        Instance['store']
      > &
        Instance['request']
  ...
  listen = (
    ...
    if (typeof options === 'string') {
      ...
      if (Number.isNaN(options)) throw new Error('Port must be a numeric value')
    }
