...
function createNode(pathPart: string, staticChildren?: any[]): Node {
    return {
        pathPart,
        store: null,
        staticChildren: staticChildren !== undefined ? new Map(staticChildren.map(child => [child.pathPart.charCodeAt(0), child])) : null,
...
export class Router {
    ...
    constructor({ storeFactory }: RouterOptions = {}) {
        if (!storeFactory) {
            ...
            storeFactory = () => {
                ...
                if (store === null) {
                    throw new Error('You should return an object from the store factory function.');
                }
                return store;
            }
        } else {
            throw new Error('The store factory should be a function that returns an object.');
        }
    ...
    register(path: string): any {
        if (typeof path !== 'string') throw new Error('The path should be a string.');
...
function matchRoute(url: string, urlLength: number, node: Node, startIndex: number): any {
    const { pathPart } = node;
